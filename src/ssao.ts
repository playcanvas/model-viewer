...
const getChunk = (name: string) => {
    // @ts-ignore
    return pc.shaderChunks[name];
}

class SSAOEffect extends pc.PostEffect {
    camera: pc.Entity;
    ssaoShader: pc.Shader;
    blurShader: pc.Shader;
    outputShader: pc.Shader;
    target: pc.RenderTarget;
    blurTarget: pc.RenderTarget;
    ...
    constructor(graphicsDevice: pc.WebglGraphicsDevice, camera: pc.Entity) {
    ...
    _resize(target: pc.RenderTarget) {
    ...
    render(inputTarget: pc.RenderTarget, outputTarget: pc.RenderTarget, rect: pc.Vec4) {
        ...
        var cameraFarClip = this.camera.camera.farClip;
